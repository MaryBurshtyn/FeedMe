...
public class History {
    ...
    public ArrayList<HistoryNote> getCurrentDay(){
        ...
        ArrayList<HistoryNote> day = new ArrayList<HistoryNote>();
        for (int i = 0; i < historyList.size(); i++){
            if(historyList.get(i).getData().equals(date)){
                day.add(historyList.get(i));
            }
        }
        return day;
    }
    public ArrayList<HistoryNote> getCurrentWeek(){
        ...
        ArrayList<HistoryNote> week = new ArrayList<HistoryNote>();
    ...
    public ArrayList<HistoryNote> getWeekHistory(){
        ...
        ArrayList<HistoryNote> weekHistory = new ArrayList<HistoryNote>();
        int portion[] = new int[7];

        for (int i = 0; i < week.size(); i++){
            switch (week.get(i).getDayOfWeek()){
                case "пн":
                    portion[0] += week.get(i).getPortion();
                    break;
                case "вт":
                    portion[1] += week.get(i).getPortion();
                    break;
                case "ср":
                    portion[2] += week.get(i).getPortion();
                    break;
                case "чт":
                    portion[3] += week.get(i).getPortion();
                    break;
                case "пт":
                    portion[4] += week.get(i).getPortion();
                    break;
                case "сб":
                    portion[5] += week.get(i).getPortion();
                    break;
                case "вс":
                    portion[6] += week.get(i).getPortion();
                    break;
            }
        }
    ...
    public ArrayList<HistoryNote> getCurrentMonth(){
        ...
        int month1 = c1.get(Calendar.MONTH)+1;
        String currentMonth;
        ArrayList<HistoryNote> month = new ArrayList<HistoryNote>();
        if(month1 < 10){
        ...
        for(int i = 0; i < historyList.size(); i++){
            if(historyList.get(i).getData().substring(3,5).equals(currentMonth))
                month.add(historyList.get(i));
        }
        return month;
    }
    public ArrayList<HistoryNote> getMonthHistory(){
        ...
        ArrayList<HistoryNote> monthHistory = new ArrayList<HistoryNote>();
        for (int i = 0; i < month.size(); i++){
            Integer day = Integer.parseInt(month.get(i).getData().substring(0,2));
            if(day <= 7) portion[0] += month.get(i).getPortion();
            if(day > 7 && day <= 14 ) portion[1] += month.get(i).getPortion();
            if(day > 14 && day <= 21 ) portion[2] += month.get(i).getPortion();
            if(day > 21 && day <= 31 ) portion[3] += month.get(i).getPortion();
        }
